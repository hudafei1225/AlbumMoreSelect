...
import com.zhihu.matisse.internal.ui.SlidingCheckLayout;
...
public class AlbumMediaAdapter extends
    ...
    public AlbumMediaAdapter(Context context, SelectedItemCollection selectedCollection, RecyclerView recyclerView, SlidingCheckLayout slidingCheckLayout) {
        super(null);
        mSelectionSpec = SelectionSpec.getInstance();
        mSelectedCollection = selectedCollection;
        mSlidingCheckLayout = slidingCheckLayout;
        mContext = context;
...
//    public Item getEntityByPosition(int position) {
//        return getCursor().get;
//    }
    ...
    protected void onBindViewHolder(final RecyclerView.ViewHolder holder, Cursor cursor) {
        ...
        mSlidingCheckLayout.setOnSlidingCheckListener(new SlidingCheckLayout.OnSlidingCheckListener() {
            @Override
            public void onSlidingCheckPos(int startPos, int endPos) {
                RecyclerView recyclerView = (RecyclerView) mSlidingCheckLayout.getChildAt(0);
//                Toast.makeText(mContext, "startPos :"+startPos+", end : "+endPos, Toast.LENGTH_SHORT).show();
                for (int i = startPos; i <= endPos; i++) {
                    cursor.moveToPosition(i);
                    Item item = Item.valueOf(getCursor());
                    ...
                    if (holder instanceof AlbumMediaAdapter.MediaViewHolder) {
//                mAdapter.setCheckStatus(item, ((AlbumMediaAdapter.MediaViewHolder) holder).mMediaGrid);
                        onCheckViewClicked(((AlbumMediaAdapter.MediaViewHolder) holder).mMediaGrid.findViewById(R.id.check_view),item, holder);
                    }
                    if (current != null) {
...
//                        }
    ...
    public void setCheckStatus(Item item, MediaGrid mediaGrid) {
